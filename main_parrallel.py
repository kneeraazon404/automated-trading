...
async def _run_model_in_current_process(
    AgentClass, model_config, INIT_DATE, END_DATE, agent_config, log_config
):
...
async def main(config_path=None, only_signature: str | None = None):
    ...
    if only_signature:
        enabled_models = [
            m for m in enabled_models if m.get("signature") == only_signature
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "config_path", nargs="?", default=None, help="Path to config JSON"
    )
    parser.add_argument(
        "--signature",
        dest="signature",
